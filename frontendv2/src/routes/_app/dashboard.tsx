...
import { ArrowUpRight, Cpu, Layers, Sparkles, Zap, CheckCircle2, XCircle, Loader2, MessageSquare } from "lucide-react";
...
function DashboardPage() {
  ...
  return (
    <div className="mx-auto w-full max-w-7xl px-6 py-10">
      ...
      {/* Chatbot launcher */}
      <Card className="mt-6 rounded-2xl border-border bg-gradient-to-br from-[#0A0A0F] to-[#1a1030] p-0 shadow-none overflow-hidden">
        <div className="flex flex-col gap-4 p-6 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-start gap-4">
            <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-[#F26B3A]/15 border border-[#F26B3A]/30">
              <MessageSquare className="h-6 w-6 text-[#F26B3A]" />
            </div>
            <div>
              <h2 className="text-base font-semibold text-white">QBETA Chatbot</h2>
              <p className="mt-0.5 text-sm text-white/60">Chat-style chip designer — describe your chip in plain English and get a fabricated layout instantly.</p>
            </div>
          </div>
          <Link to="/chat" className="inline-flex h-10 shrink-0 items-center gap-2 rounded-full bg-[#F26B3A] px-5 text-sm font-semibold text-white transition-all hover:bg-[#F26B3A]/85">
            Open Chatbot <ArrowUpRight className="h-3.5 w-3.5" />
          </Link>
        </div>
      </Card>

      <Card className="mt-4 rounded-2xl border-border p-8 shadow-none">
